...
setValues()
{
 console.log(this.isChecked);
  ...
  //  var id = JSON.parse(localStorage.getItem("_logedinuser_"));
...
setRentLocation(event: any) {
  ...
  onStateRent(stateid) {
    ...
    for (var x in this.states[stateid.target.selectedIndex].cities) {
      this.dispCityRental.push(this.states[stateid.target.selectedIndex].cities[x]);
    }
 console.log(this.states);
  ...
  onStateReturn(stateid) {
    ...
    for (var x in this.states[stateid.target.selectedIndex].cities) {
      this.dispCityReturn.push(this.states[stateid.target.selectedIndex].cities[x]);
    }
  ...
  onCityRent(cityid) {
    ...
    var state_index = this.states.findIndex(x => x.stateid == this.PStateidRent);

    console.log("state id" + this.PStateidRent + "cityid" + this.indexRent);

    console.log(this.states[0].cities[1].hubs[1].hubaddress);

    for (let c in this.states[state_index].cities[this.indexRent].hubs) {
      this.dispHubRent.push(this.states[state_index].cities[this.indexRent].hubs[c].hubaddress);
    }
  ...
  onCityReturn(cityid) {
    ...
    var state_index = this.states.findIndex(x => x.stateid == this.PStateidReturn);
    console.log("state id" + this.PStateidReturn + "cityid" + this.indexReturn);

    console.log(this.states[0].cities[1].hubs[1].hubaddress);

    for (let c in this.states[state_index].cities[this.indexReturn].hubs) {
      this.dispHubReturn.push(this.states[state_index].cities[this.indexReturn].hubs[c].hubaddress);
    }
